...
- [2026-04-17] В `@keyframes yz-vanish` финальный кадр должен зануллять не только `max-height`, но и `width` / `max-width` / `min-width` / `flex-basis`. Без этого скрытый `ytd-rich-section-renderer` (Shorts shelf новой раскладки) сохраняет 100% ширину flex-контейнера `#contents` и переносится flex-wrap на новый row, оставляя видимый пустой слот в предыдущем row. Подтверждено Claude-in-Chrome DOM-инспекцией.
