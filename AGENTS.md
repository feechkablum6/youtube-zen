...
- [2026-04-16] Новые CSS-селекторы для YouTube верифицировать через Codex-in-Chrome MCP на залогиненном браузере пользователя, а не через отдельный DevTools (там нет сессии).
...
- [2026-06-03] YouTube заменил в секции «Навигатор» пункт «Видеоигры» (`/gaming`) на «Фильмы» (`/feed/storefront`), а в «Другие возможности» убрал Premium (`/premium`), оставив YouTube Music (`https://music.youtube.com`) и YouTube Детям (`youtubekids.com`). Секции теперь идентифицируются по этим ссылкам; `/gaming` и `/premium` оставлены как A/B fallback.
- [2026-06-03] Списки сайдбара «Вы»/«Навигатор»/«Другие возможности» — вложенные тумблеры (`SIDEBAR_LISTS` в `selectors.ts`), а не плоские правила: master-ключ + дети. Семантика в `buildCss`: master OFF → ничего; все дети ON → `sectionSelectors` (вся секция с заголовком); частично → селекторы включённых детей. Ключи `playlists/liked/yourVideos/downloads` (дети «Вы») и `navigator/explore` (master) переиспользованы из старой плоской схемы — миграции storage нет.
- [2026-04-21] В обработчике `yt-navigate-start` `window.location.href` ещё указывает на СТАРЫЙ URL — целевой URL приходит в `event.detail.url` (относительный путь). И `history.replaceState` после события YouTube перезаписывает своим навигатором (Navigation API, без pushState). Чтобы подменить URL до навигации — мутировать `event.detail.url` (и `event.detail.endpoint.commandMetadata.webCommandMetadata.url` для симметрии). Применено в `src/content/filters/search-url-rewriter.ts`.
- [2026-06-05] Кнопка поиска в masthead может быть новой разметкой `button.ytSearchboxComponentSearchButton`, а не старым `#search-icon-legacy`. Inline-кнопку фильтров монтировать с поддержкой обоих селекторов и проверять через залогиненный YouTube DOM.
- [2026-06-05] Inline-фильтр «Дата загрузки» применяет текущую страницу `/results` сразу через `window.location.assign(...)`: существующий `sp` считается управляемым модалкой, заменяется новым или удаляется при возврате к `any`. Остальные search-селекты пока только сохраняются для будущих переходов.
- [2026-06-05] При поиске с главной YouTube может уже передать `sp` в `yt-navigate-start`; если search-фильтр активен, `rewriteIfNeeded` обязан заменить этот `sp`, а не пропустить URL из-за наличия параметра.
- [2026-06-05] Для поиска с главной `yt-navigate-start` не всегда достаточно: живой YouTube может завершить SPA-переход без изменённого `sp`. Поэтому active upload-date filter дополнительно проверяется на `yt-navigate-finish` и при необходимости делает `window.location.assign(...)` на `/results` с актуальным `sp`.
- [2026-06-05] `yt-navigate-finish` fallback обязательно должен быть one-shot на конкретный target URL (`sessionStorage` guard). Без этого YouTube/URL-кодирование `sp` может вызвать бесконечный цикл `window.location.assign(...)`.
- [2026-06-05] Чтобы search-фильтр открывался без промежуточной стандартной выдачи, перехватывать search-intent в capture-фазе (`submit`, Enter в `input[name="search_query"]`, клик по `button.ytSearchboxComponentSearchButton` / `#search-icon-legacy`) и сразу запускать SPA-навигацию YouTube на `/results` с `sp`; `window.location.assign(...)` допускается только как fallback.
- [2026-06-05] Content script не может напрямую вызвать `ytd-app.handleNavigate` в page world: direct-вызов из isolated world падает в `window.location.assign` и вызывает `beforeunload`. Для search-intent использовать bridge `src/content/page-bridge.ts` как `web_accessible_resources`: content dispatches `yz:search-navigate`, page bridge вызывает `handleNavigate`, затем отвечает `yz:search-navigate-result`. Проверять отсутствие reload по отсутствию `beforeunload` и наличию `yt-navigate-start/finish`.
- [2026-06-05] На странице поиска `/results` полка Shorts рендерится как `grid-shelf-view-model:has(ytm-shorts-lockup-view-model-v2)`, без `ytd-reel-shelf-renderer`. Правило `shorts` должно таргетировать этот контейнер, иначе заголовок и ряд шортсов остаются видимыми.
