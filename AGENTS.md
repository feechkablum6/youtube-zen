...
- [2026-06-05] Чтобы search-фильтр открывался без промежуточной стандартной выдачи, перехватывать search-intent в capture-фазе (`submit`, Enter в `input[name="search_query"]`, клик по `button.ytSearchboxComponentSearchButton` / `#search-icon-legacy`) и сразу делать `window.location.assign(...)` на `/results` с `sp`; `yt-navigate-start/finish` остаются страховкой.
